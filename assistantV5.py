...
def fetch_working_hours_and_energy_levels(sheets_service, weather_analysis=False):
    ...
    try:
        ...
        end_date = today + timedelta(days=27)  # 28 days including today

        # Generate the list of dates for the next 28 days
        date_list = [today + timedelta(days=i) for i in range(28)]
        day_names = [date.strftime("%A") for date in date_list]

        # Initialize the energy profile with empty lists for each day
        ...
        for row in rows:
            if len(row) >= 4:
                ...
                # Validate that the day name is correct and exists in the next 28 days
                if day_name not in day_names:
                    print(f"Skipping row with day name not in the next 28 days: {day_name}")
                    continue

                # Calculate the date for the given day name within the range of the next 28 days
                ...
                if end_time <= start_time:
                    ...
                    # Add the first part of the overnight slot
                    ...
                    # Add the second part of the overnight slot
                    next_day_name = next_day.strftime("%A")
                    if next_day_name in energy_profile:
